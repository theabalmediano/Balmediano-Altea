...
<style>
body { font-family: 'Poppins', sans-serif; background: url('https://i.pinimg.com/736x/bf/01/f9/bf01f9444340ecdb37af06d201c6f1cf.jpg') center/cover fixed; }
.overlay { background: rgba(255, 240, 245, 0.7); }
</style>
</head>
<body class="min-h-screen relative flex flex-col items-center py-8 text-gray-800">
            ...
            <?php if(!empty($users)): ?>
                ...
                <tr class="odd:bg-white/20 even:bg-white/10 hover:bg-pink-50/20 transition-colors">
                    ...
                    <td class="px-4 py-2 flex justify-center gap-2">
                        ...
                        <form action="<?= site_url('users/delete/'.$user['id']); ?>" method="post" onsubmit="return confirm('Are you sure?')" class="inline">
                            <button type="submit" class="px-2 py-1 bg-red-500 rounded-lg text-white hover:bg-red-600">
                                <i class="fa-solid fa-trash"></i>
                            </button>
                        </form>

                        <form action="<?= site_url('users/soft_delete/'.$user['id']); ?>" method="post" onsubmit="return confirm('Soft delete this user?')" class="inline">
                            <button type="submit" class="px-2 py-1 bg-pink-500 rounded-lg text-white hover:bg-pink-600">
